...
public class ExtStorage {
    ...
    /*
     * public long getMax(boolean limit){ return computeMax(values,limit); }
     */
     ...
     * java.util.Arrays.sort(values); int limit = 0; if(percent){ limit =
     * values.length/10; } long max = 0; for(int i = limit; i < values.length -
     * limit;i++){ if (values[i]>max){ max = values[i]; } } return max; }
     */
