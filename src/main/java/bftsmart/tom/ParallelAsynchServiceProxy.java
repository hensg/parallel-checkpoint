...
public class ParallelAsynchServiceProxy extends AsynchServiceProxy {
    ...
    public ParallelAsynchServiceProxy(int processId, String configHome, Comparator<byte[]> replyComparator,
     ...
     * @return
     */
    public int invokeParallelAsynchRequest(byte[] request, ReplyListener replyListener, TOMMessageType reqType,
            int groupId) {
        return invokeParallelAsynchRequest(request, super.getViewManager().getCurrentViewProcesses(), replyListener,
                reqType, groupId);
    }
    ...
    public int invokeParallelAsynchRequest(byte[] request, int[] targets, ReplyListener replyListener,
            TOMMessageType reqType, int groupId) {
    ...
    private int invokeAsynch(byte[] request, int[] targets, ReplyListener replyListener, TOMMessageType reqType,
        ...
        requestContext = new RequestContext(generateRequestId(reqType), generateOperationId(), reqType, targets,
                System.currentTimeMillis(), replyListener);
