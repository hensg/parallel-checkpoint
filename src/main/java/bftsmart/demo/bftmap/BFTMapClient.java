	...
	public static void main(String[] args) {
		...
		while (true) {
			try {
				...
				boolean result;
				if (ops % 10000 == 0) {
					// System.out.println("TABLE SIZE = "+getSizeTable(bftMap,"table"));
					System.out.println(ops);
					tableName = "table" + Integer.toString(ops);
					createTable(bftMap, tableName);
					result = true;
					System.out.println("CREATED TABLE" + tableName);
					// createTable(bftMap, "table"+ops);
				} else {
					result = insertValue(bftMap, tableName, ops);
				}
				...
				// Sys btem.out.println("ops sent: "+ops);
				ops++;
