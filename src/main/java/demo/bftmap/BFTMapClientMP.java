...
public class BFTMapClientMP {
    ...
    public static void main(String[] args) throws IOException {
        if (args.length < 7) {
            System.out.println("Usage: ... ListClient <num. threads> <process id> <number of operations> <interval> <maxIndex> <p_read %> <p_conflict %> <verbose?> <parallel?> <async?>");
            System.exit(-1);
        }
        ...
        for (int i = 0; i < numThreads; i++) {
            ...
            c[i] = new BFTMapClientMP.Client(initId + i, numberOfOps, interval, max, verbose, parallel, async,numThreads,p_read,p_conflict);
            //c[i].start();
        }
...
//        }
    ...
    static class Client extends Thread {
       ...
       // int percent;
        ...
        public Client(int id, int numberOfOps, int interval, int maxIndex, boolean verbose, boolean parallel, boolean async,int numThreads,int p_read,int p_conflict) {
            super("Client " + id);
            this.id = id;
...
//                                  }  
            ...
            if (id == initId) {
                ...
                //System.out.println(this.id + " // 90th percentile for " + numberOfOps + " executions = " + store.getPercentile(90) / 1000 + " us ");
...
//		System.out.println("resultBytes" + resultBytes);
        ...
        private static byte[] putEntries(PBFTMapMP bftMap, Integer nameTable1, Integer key1,Integer nameTable2, Integer key2) throws Exception {
		Integer k1 = key1;
...
//		System.out.println("resultBytes" + resultBytes);
        ...
        private static byte[] getEntries(PBFTMapMP bftMap, Integer tableName1, Integer key1,Integer tablename2, Integer key2){
