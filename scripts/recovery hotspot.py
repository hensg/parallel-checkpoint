...
for path in Path(args.dir).rglob("**/**/*server*.log"):
    ...
    with open(path) as file:
        for line in file:
            ...
            g = re.findall(
                "(\d+-\d+-\d+ [0-9]{2}:[0-9]{2}:[0-9]{2}.[0-9]{3}).*Scheduled recovery finished message for partition ([0-9]+)",
                line,
            )
            if g and (
                    datetime.strptime(g[0][0], TIMEFORMAT)
...
requesting_cp=[
    request_response_tt_cp["true"]["4"]["2000000"][0],
    request_response_tt_cp["true"]["8"]["2000000"][0],
    request_response_tt_cp["true"]["16"]["2000000"][0],

]

installing_cp=[
    install_snapshot_tt["true"]["4"]["2000000"][0],
    install_snapshot_tt["true"]["8"]["2000000"][0],
    install_snapshot_tt["true"]["16"]["2000000"][0],
]

requesting_logs=[
    req_res_logs["true"]["4"]["2000000"][0],
    req_res_logs["true"]["8"]["2000000"][0],
    req_res_logs["true"]["16"]["2000000"][0],
]

installing_logs=[
    install_logs_tt["true"]["4"]["2000000"][0],
    install_logs_tt["true"]["8"]["2000000"][0],
    install_logs_tt["true"]["16"]["2000000"][0],
]
